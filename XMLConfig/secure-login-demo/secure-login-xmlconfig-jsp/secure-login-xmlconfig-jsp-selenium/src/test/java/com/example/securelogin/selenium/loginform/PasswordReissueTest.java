...
public class PasswordReissueTest extends DBLogFunctionTestSupport {
    ...
    public void testPasswordReissue001() throws IOException, InterruptedException {
        ...
        page = new PasswordReissuePage(webDriverOperations, applicationContextUrl, url).open();

        // reissue password
        page = ((PasswordReissuePage) page).reissueSuccess(secret, "Hoge1", "Hoge1");
        assertTrue(webDriverOperations.getCurrentUrl().endsWith("/resetpassword?complete"));
        ...
        page = ((LoginPage) new LoginPage(webDriverOperations, applicationContextUrl).open())
                .loginSuccess("demo", "Hoge1").gotoTop();
    ...
    public void testPasswordReissue003() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < tokenValidityThreshold; i++) {
            page = ((PasswordReissuePage) page).reissueFailure("aaaaa", "Hoge2", "Hoge2");
        }
