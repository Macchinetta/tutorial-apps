...
import com.example.securelogin.selenium.loginform.page.mail.ReceivedMailPage;
import com.example.securelogin.selenium.loginform.page.passwordreissue.CreateReissueInfoSuccessPage;
import com.example.securelogin.selenium.loginform.page.passwordreissue.PasswordReissuePage;
...
public class PasswordReissueTest extends DBLogFunctionTestSupport {
    ...
    public void testPasswordReissue001() throws IOException, InterruptedException {
        ...
        page = ((LoginPage) page).goToCreateReissueInfoPage().makeReissueInfo(
                "demo");
        assertTrue(!((CreateReissueInfoSuccessPage) page).getSecret()
                .isEmpty());
        ...
        page = new PasswordReissuePage(webDriverOperations, applicationContextUrl, url)
                .open();
        ...
        page = ((LoginPage) new LoginPage(webDriverOperations, applicationContextUrl)
                .open()).loginSuccess("demo", "Hoge1").gotoTop();
    ...
    public void testPasswordReissue002() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < n; i++) {
            page = ((LoginPage) new LoginPage(webDriverOperations, applicationContextUrl)
                    .openWithDescription(
                            "secret phrase and URL is generated at random"))
    ...
    public void testPasswordReissue003() throws IOException, InterruptedException {
        AbstractPageObject page = new LoginPage(webDriverOperations, applicationContextUrl)
                .openWithDescription(
                        "the URL is invalidated by series of incorrect password attempts");
        ...
        for (int i = 0; i < tokenValidityThreshold; i++) {
            page = ((PasswordReissuePage) page).reissueFailure("aaaaa", "Hoge2",
                    "Hoge2");
        }
        assertThat(webDriverOperations.getTitle(), is("Business Error!"));
    }
    ...
    public void testPasswordReissue004() throws IOException, InterruptedException {
        AbstractPageObject page = new LoginPage(webDriverOperations, applicationContextUrl)
                .openWithDescription(
                        "the URL is invalidated by passage of time");
        ...
        page = new PasswordReissuePage(webDriverOperations, applicationContextUrl, url)
                .open();
        assertThat(webDriverOperations.getTitle(), anyOf(is(
                "Resource Not Found Error!"), is("Business Error!")));
    }
}
