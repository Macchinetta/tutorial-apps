...
import com.example.todo.config.SeleniumContextRestConfig;
import com.example.todo.selenium.RestTestSupport;
import io.restassured.RestAssured;
import io.restassured.http.ContentType;

@ContextConfiguration(classes = {SeleniumContextRestConfig.class})
    ...
    public void testPutErr405() throws Exception {
        ...
        RestAssured.given().body(jsonBody).contentType(ContentType.JSON).when().put("").then()
                .statusCode(405).body("code", equalTo("E999")).body("message", equalTo(
                        "[E999] Error occurred. Caused by : Request method 'PUT' is not supported"));
    }
