...
public class AccountLockTest extends DBLogFunctionTestSupport {
    ...
    public void testAccountLock001() throws IOException, InterruptedException {
        ...
        assertThat(((LoginPage) page).getLoginError(), is("User account is locked"));
    ...
    public void testAccountLock002() throws IOException, InterruptedException {
        ...
        page = ((LoginPage) page).loginSuccessIntercepted("admin", "demo")
                .changePasswordSuccess("demo", "Bar1", "Bar1").gotoTop().goToUnlockPage()
                .unlockSuccess("demo");
        assertTrue(webDriverOperations.getCurrentUrl().endsWith("unlock?complete"));

        // confirm the account is successfully unlocked
        page = ((UnlockSuccessPage) page).gotoTop().logout().loginSuccessIntercepted("demo",
                "demo");
        assertTrue(webDriverOperations.getCurrentUrl().endsWith("/password?form"));
        page = ((PasswordChangePage) page).changePasswordSuccess("demo", "Foo1", "Foo1").gotoTop()
                .logout();
